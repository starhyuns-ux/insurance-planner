...
import RewardWalletSuccess from './RewardWalletSuccess'
...
export default function ConsultationForm({ id = "consultation", plannerId, plannerInfo }: ConsultationFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // 추가 필드 검증 (리워드 조건)
    if (!birthDate.trim()) {
      setError(t('formAlertBirthDate'))
      return
    }
    if (!monthlyPremium.trim()) {
      setError(t('formAlertPremium'))
      return
    }
    ...
    try {
      ...
      if (data.success) {
        // 성공 시 데이터 백업 후 지갑 상태로 전환
        setSubmittedCode(data.referral_code)
        setSubmittedName(name)

        // Form reset (in case user goes back somehow)
        setName('')
        setPhone('')
        setBirthDate('')
        setMonthlyPremium('')
            ...
            <div className="space-y-5 text-sm font-medium relative z-10">
              <div className="flex items-center gap-4">
                <div className="w-10 h-10 rounded-full bg-white/10 flex items-center justify-center shrink-0">
                  <span className="text-primary-400 text-lg font-black">1</span>
                </div>
                <span>{t('formStep1')}</span>
              </div>
              <div className="flex items-center gap-4">
                <div className="w-10 h-10 rounded-full bg-white/10 flex items-center justify-center shrink-0">
                  <span className="text-primary-400 text-lg font-black">2</span>
                </div>
                <span>{t('formStep2')}</span>
              </div>
              <div className="flex items-center gap-4">
                <div className="w-10 h-10 rounded-full bg-white/10 flex items-center justify-center shrink-0">
                  <span className="text-primary-400 text-lg font-black">3</span>
                </div>
