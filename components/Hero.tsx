    ...
    <section className="relative pt-20 pb-16 md:pt-32 md:pb-24 overflow-hidden">
      ...
      <div className="container flex items-center justify-center">
        ...
        <div className="max-w-3xl px-4 md:px-0 text-center">
          {planner ? (
            <div className="inline-flex items-center gap-2 px-5 py-2.5 rounded-full bg-white shadow-xl shadow-primary-100 border border-primary-100 mb-8 animate-in slide-in-from-top-4 duration-700">
              <div className="w-8 h-8 rounded-full overflow-hidden border-2 border-primary-200">
                {planner.profile_image_url ? (
                  ...
                  <div className="w-full h-full bg-primary-600 flex items-center justify-center text-[10px] text-white font-black">
                    {planner.name[0]}
                  </div>
                )}
              </div>
              <div className="text-left">
                <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest leading-none mb-0.5">Verified Advisor</p>
                <p className="text-xs font-black text-gray-900">{planner.name} 설계사와 함께하는 인슈닷</p>
              </div>
          ...
          <h1 className="text-[2.5rem] md:text-5xl lg:text-6xl font-extrabold text-gray-900 tracking-tight leading-[1.25] mb-6 break-keep">
            {planner ? (
              <>
                <span className="text-primary-600">{planner.name} 전문가</span>가 제안하는<br />
                스마트한 보험 생활
              </>
            ) : (
              <>
