...
export default function Overview({ leads, customers, planner, totalVisits, visitStats, getInsuranceAge }: OverviewProps) {
  ...
  return (
    <div className="space-y-12">
      ...
      {/* Today's Leads */}
      <div className="bg-white rounded-[2rem] shadow-xl p-6 border border-gray-100 flex flex-col justify-between transition-all hover:shadow-2xl hover:-translate-y-1">
        ...
        <div>
          <h4 className="text-3xl font-black text-gray-900 mb-1">{todayLeads}건</h4>
          <p className="text-[10px] text-gray-400 font-bold tracking-tight mt-1 uppercase">Today's New Leads</p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-[2rem] shadow-xl p-6 border border-gray-100 flex flex-col justify-between transition-all hover:shadow-2xl hover:-translate-y-1">
        ...
        <div>
          <h4 className="text-3xl font-black text-gray-900 mb-1">{(planner?.visit_count || 0).toLocaleString()}회</h4>
          <p className="text-[10px] text-gray-400 font-bold tracking-tight mt-1 uppercase">Cumulative Site Views</p>
        </div>
      </div>

      {/* Traffic Stats */}
      <div className="bg-white rounded-[2rem] shadow-xl p-6 border border-gray-100 flex flex-col justify-between overflow-hidden relative group transition-all hover:shadow-2xl hover:-translate-y-1">
        <div className="flex items-center justify-between mb-4">
        ...
        <div>
          <h4 className="text-3xl font-black text-gray-900 mb-1">{totalVisits.toLocaleString()}회</h4>
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
      ...
      <div className="bg-gradient-to-br from-rose-50 to-white rounded-[2rem] shadow-xl p-8 border border-rose-100 flex items-center justify-between group transition-all hover:shadow-2xl hover:-translate-y-1">
        <div className="flex flex-col">
          <span className="text-[10px] font-black text-rose-600 uppercase tracking-[0.15em] mb-1 flex items-center gap-2">
            <div className="w-1.5 h-1.5 bg-rose-500 rounded-full animate-pulse" />
            관리가 시급한 고객
          </span>
          <h4 className="text-2xl font-black text-gray-900 leading-tight">
            {needsManagementCount} <span className="text-xs font-bold text-gray-400 ml-1">명</span>
          </h4>
          <p className="text-[10px] text-gray-400 font-bold mt-1 break-keep">30일 이상 연락이 닿지 않은 고객</p>
        </div>
      ...
      <div className="bg-gradient-to-br from-primary-50 to-white rounded-[2rem] shadow-xl p-8 border border-primary-100 flex items-center justify-between group transition-all hover:shadow-2xl hover:-translate-y-1">
        <div className="flex flex-col">
          <span className="text-[10px] font-black text-primary-600 uppercase tracking-[0.15em] mb-1 flex items-center gap-2">
            <div className="w-1.5 h-1.5 bg-primary-500 rounded-full animate-pulse" />
            보험료 인상 임박 (상령일)
          </span>
          <h4 className="text-2xl font-black text-gray-900 leading-tight">
            {upcomingDDayCount} <span className="text-xs font-bold text-gray-400 ml-1">명</span>
          </h4>
          <p className="text-[10px] text-gray-400 font-bold mt-1 break-keep">14일 이내 상령일 도래 (집중 공략 권장)</p>
        </div>
